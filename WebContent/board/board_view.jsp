...
<script type="text/javascript" src="http://code.jquery.com/jquery-1.4.4.min.js"></script>
	<script type="text/javascript">
		...
		function update() {
			...
			// post방식
			document.form1.action.value="viewUpdate";
			document.form1.submit();
		}
  ...
  <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
   <ul class="nav navbar-nav">
	...
	<form name="form1" method="post" action=board_control.jsp>
		...
		<thead>
 			<tr>
 				<th colspan="3" style = "background-color :#eeeeee; text-align: center;">Q&A 글보기 </th>
 			</tr>
 		</thead>
		<%if(action.equals("add")) { %>
		<tbody>
		<tr>
			...
			<tr>
				<th>비번확인</th>
				<td><input class="form-control" type=password size=100 id="reppasswd" name=reppasswd value=""></td>
			</tr>
			</tbody>
			<%}
		      }else{ %>
		<tbody>
			<tr>
				<th width="15%" style = "text-align : center;">제목</th>
				<td style="border-right: 1px solid #dddddd; text-align:left;"><%= boardDTO.getTitle() %></td>				
			</tr>
			<tr>
				<th width="15%" style = "text-align : center;">성명</th>
				<td style="border-right: 1px solid #dddddd; text-align:left;"><%= boardDTO.getUserName() %></td>
			</tr>
			<tr>
				<th width="15%" style = "text-align : center;">내용</th>
				<td style="height: 350px; border-right: 1px solid #dddddd; text-align:left;"><%= boardDTO.getContent() %></td>
			</tr>
			</tbody>
			<%} %>
		...
		<%if(action.equals("add")) { %>	
			<input style="margin-right: 3px;" class="btn btn-primary pull-right" type="button" id="insert" value="입력" onClick="insertcheck()">		
	    <%}else{ %>
			<input style="margin-right: 3px;" class="btn btn-primary pull-right" type="button" id="reply" value="답변" onClick="replycheck()">
			<input style="margin-right: 3px;" class="btn btn-primary pull-right" type="button" id="updatev" value="수정" onClick="update()">
		<%} %>
	</form>
	</div>
	</div>
