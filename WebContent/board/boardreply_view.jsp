...
<script type="text/javascript" src="http://code.jquery.com/jquery-1.4.4.min.js"></script>
	<script type="text/javascript">
		...
		function re_update() {
			...
			// post방식
			document.form1.action.value="re_updatev";
			document.form1.submit();
		}
  ...
  <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
   <ul class="nav navbar-nav">
	...
	<form name="form1" method="post" action=board_control.jsp>
		...
		<thead>
 			<tr>
 				<th colspan="3" style = "background-color :#eeeeee; text-align: center;">Q&A 댓글 추가</th>
 			</tr>
 		</thead>
 		<%if(action.equals("add")) {%>
		<tbody>
		...
		<a href="board_list.jsp" class="btn btn-primary pull-right">목록으로 돌아가기</a>
		<%if(action.equals("add")) { %>	
			<input style="margin-right: 3px;" class="btn btn-primary pull-right" type="button" id="insert" value="입력" onClick="insertcheck()">		
	    <%}else{ %>
			<input style="margin-right: 3px;" class="btn btn-primary pull-right" type="button" id="updatev" value="수정" onClick="re_update()">
		<%} %>
	</form>
	</div>
	</div>	
